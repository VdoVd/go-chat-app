...
func recvProc(node *Node) {
	for {
		...
		if msg.Type == 3 {
			...
			dispatch(data)
			broadMsg(data)
			fmt.Println("[ws] recvProv <<<<<", string(data))
		}
	}
}
...
func sendMsg(userId int64, msg []byte) {
	...
	r, err := utils.Redis.Get("online_" + userIdStr).Result()
	if err != nil {
		fmt.Println(err)
	}
	if r != "" {
	...
	res, err := utils.Redis.ZRevRange(key, 0, -1).Result()
	if err != nil {
		fmt.Println(err)
	}
	score := float64(cap(res)) + 1
	ress, e := utils.Redis.ZAdd(key, &redis.Z{score, msg}).Result() //jsonMsg
...
func RedisMsg(userIdA int64, userIdB int64, start int64, end int64, isRev bool) []string {
	...
	var err error
	if isRev {
		rels, err = utils.Redis.ZRange(key, start, end).Result()
	} else {
		rels, err = utils.Redis.ZRevRange(key, start, end).Result()
	}
	if err != nil {
